...
| thresholdUnit        | Threshold unit. Can be either empty or "%". 
If empty, values are compared ( absolute_value(oldValue - newValue) > threshold ). 
If %, percentage are calculated before comparing values  ( absolute_value(oldValue - newValue) > (oldValue * threshold/100) ). |
