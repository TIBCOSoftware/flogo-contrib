...
func (pi *Instance) Start(startAttrs []*data.Attribute) bool {
	...
	if ok {
		...
		for _, taskEntry := range taskEntries {
			...
			enterTaskData, _ := pi.FlowTaskEnv.FindOrCreateTaskData(taskEntry.Task)
		...
		//rootTaskData := pi.FlowTaskEnv.NewTaskData(pi.Flow.RootTask())
...
//<<<<<<< HEAD
			...
			if workItem.TaskData.taskEnv.ID != idEhTaskEnv {
//=======
...
func (pi *Instance) handleTaskDone(taskBehavior model.TaskBehavior, taskData *TaskData, doneCode int) {
	...
	//		parentDone, parentDoneCode := parentBehavior.ChildDone(parentTaskData, task, notifyCode)
	//
	//		if parentDone {
	...
	//		flowBehavior := pi.FlowModel.GetFlowBehavior()
	//		flowBehavior.TasksDone(pi, notifyCode)
...
//<<<<<<< HEAD
		pi.appendErrorData(err)
		if taskData.taskEnv.ID != idEhTaskEnv {
//=======
...
func (pi *Instance) HandleGlobalError() {
	...
	if pi.Flow.GetErrorHandlerFlow() != nil {
		...
		if pi.EhFlowTaskEnv == nil {
			var taskEnv TaskEnv
			taskEnv.ID = idEhTaskEnv
			//taskEnv.Task = ehTask
		...
		if ok {
			...
			for _, taskEntry := range taskEntries {
				...
				enterTaskData, _ := pi.EhFlowTaskEnv.FindOrCreateTaskData(taskEntry.Task)
...
func (te *TaskEnv) FlowDefinition() *definition.Definition {
	return te.flowDef
}

// TaskInsts get the task instances
func (te *TaskEnv)  TaskInsts() []model.TaskInst {
